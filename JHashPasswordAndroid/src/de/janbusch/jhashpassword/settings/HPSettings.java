...
public class HPSettings extends Activity {
	...
	public boolean onButtonClicked(View btn) {
			...
			inputDialog.setPositiveButton(getString(R.string.OK),
					new OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							String newHostName = etInput.getText().toString()
									.trim();

							if (hashPassword.getHosts().getHostByName(
									newHostName) == null
									&& newHostName.length() > 0) {
								Host newHost = new Host();
								newHost.setName(newHostName);
								newHost.setCharset(hashPassword
										.getDefaultCharset());
								newHost.setPasswordLength(hashPassword
										.getDefaultPasswordLength());
								newHost.setHashType(hashPassword
										.getDefaultHashType());
		...
		case R.id.btnRemHost:
			if (sprHostname.getSelectedItem() != null) {
				currentHost = hashPassword.getHosts().getHostByName(
		...
		case R.id.btnEditHost:
			if (sprHostname.getSelectedItem() != null) {
				currentHost = hashPassword.getHosts().getHostByName(
						sprHostname.getSelectedItem().toString());
				if (currentHost != null) {
					Intent editHostIntent = new Intent(getBaseContext(),
							HPSettingsHost.class);
					editHostIntent.putExtra(getString(R.string.host),
							currentHost);
					editHostIntent.putExtra(getString(R.string.hp),
							hashPassword);
				...
				if (currentHost != null) {
					if (sprLoginname.getSelectedItem() != null) {
						currentLogin = currentHost.getLoginNames()
								.getLoginNameByName(
										sprLoginname.getSelectedItem()
												.toString());
						if (currentLogin != null) {
							Intent editLoginIntent = new Intent(
									getBaseContext(), HPSettingsHost.class);
							editLoginIntent.putExtra(getString(R.string.host),
									currentHost);
							editLoginIntent.putExtra(getString(R.string.login),
									currentLogin);
							editLoginIntent.putExtra(getString(R.string.hp),
									hashPassword);
				...
				inputDialog.setPositiveButton(getString(R.string.OK),
						new OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								String newLoginName = etInput.getText()
										.toString().trim();

								if (currentHost != null
										&& newLoginName.length() > 0) {
									if (currentHost.getLoginNames()
											.getLoginNameByName(newLoginName) == null) {
		...
		case R.id.btnRemLogin:
			if (sprHostname.getSelectedItem() != null) {
				currentHost = hashPassword.getHosts().getHostByName(
						sprHostname.getSelectedItem().toString());
				if (currentHost != null
						&& sprLoginname.getSelectedItem() != null) {
					currentLogin = currentHost.getLoginNames()
							.getLoginNameByName(
									sprLoginname.getSelectedItem().toString());
					if (currentLogin != null) {
